      ...
      <div className="bg-white dark:bg-slate-800 rounded-2xl shadow-sm border border-slate-200 dark:border-slate-700 overflow-hidden transition-all duration-300">
        ...
        >
          <div className="flex items-center gap-3">
            ...
            <div>
              <h3 className="text-lg font-bold text-slate-900 dark:text-white">{t('privacyTitle')}</h3>
              <p className="text-sm text-slate-500 dark:text-slate-400">{t('privacySubtitle')}</p>
            </div>
          </div>
          {isPrivacyOpen ? <ChevronUp className="w-5 h-5 text-slate-400" /> : <ChevronDown className="w-5 h-5 text-slate-400" />}
        </button>
        
        {isPrivacyOpen && (
          <div className="px-6 pb-8 pt-2 border-t border-slate-100 dark:border-slate-700/50 animate-in slide-in-from-top-2 fade-in duration-300">
            <div className="flex items-start gap-3">
